console.log("Technology Compass page loaded.");
...
function initializeTechnologyCompass() {
  ...
  const techOrder = [
    ...
    { key: "ubuntu", ring: 2, angle: 45, radius: 170, label: "Ubuntu" },
    { key: "rocky", ring: 2, angle: 225, radius: 170, label: "Rocky Linux" },
  ...
  function renderOverviewState() {
    ...
    projectFeatures.innerHTML = "";
    overviewState.projectFeatures.forEach(function (feature) {
      const li = document.createElement("li");
      li.textContent = feature;
      projectFeatures.appendChild(li);
    });
  ...
  function renderSkill(skillKey) {
    ...
    skill.projects.forEach(function (project, index) {
      ...
      card.addEventListener("click", function () {
        document.querySelectorAll(".project-preview-card").forEach(function (item) {
  ...
  function applyRingRotations() {
    document.querySelectorAll(".orbit-skill").forEach(function (node) {
      const ring = node.dataset.ring;
      node.style.setProperty("--ring-rotation", ringRotation[ring] + "deg");
    });

    document.querySelectorAll(".orbit-particle").forEach(function (particle) {
  ...
  orbitShell.addEventListener("click", function (event) {
    if (event.target.closest(".orbit-skill")) {
      return;
    }
