...
var UnknownWellKnown = Symbol("Unknown Well-Known");
var SymbolToStringTag = Symbol("Symbol.toStringTag");
var SymbolDispose = Symbol("Symbol.asyncDispose");
var SymbolAsyncDispose = Symbol("Symbol.dispose");
var SymbolHasInstance = Symbol("Symbol.hasInstance");
var WELL_KNOWN_SYMBOLS_LIST = [
  SymbolToStringTag,
  SymbolDispose,
  SymbolAsyncDispose,
  SymbolHasInstance
];
var WELL_KNOWN_SYMBOLS = new Set(WELL_KNOWN_SYMBOLS_LIST);
var isAnyWellKnown = (v) => WELL_KNOWN_SYMBOLS.has(v) || v === UnknownWellKnown;
var TREE_SHAKEABLE_SYMBOLS_LIST = [SymbolHasInstance, SymbolDispose, SymbolAsyncDispose];
var TREE_SHAKEABLE_SYMBOLS = new Set(TREE_SHAKEABLE_SYMBOLS_LIST);
var isConcreteKey = (v) => typeof v === "string" || WELL_KNOWN_SYMBOLS.has(v);
var EMPTY_PATH = [];
var UNKNOWN_PATH = [UnknownKey];
var UNKNOWN_NON_ACCESSOR_PATH = [UnknownNonAccessorKey];
var UNKNOWN_INTEGER_PATH = [UnknownInteger];
var INSTANCEOF_PATH = [SymbolHasInstance];
...
var ExternalModule = class {
  constructor(options, id, moduleSideEffects, meta, renormalizeRenderPath, attributes) {
    ...
    this.importersByExportedName = /* @__PURE__ */ new Map();
  ...
  warnUnusedImports() {
    ...
    for (const name of unused) {
      ...
      for (const importer of this.importers) {
        if (!(importersOfName == null ? void 0 : importersOfName.has(importer)))
          continue;
...
function isExpressionStatementNode(node) {
  return node instanceof NodeBase && node.type === ExpressionStatement;
}
...
var ObjectEntity = class extends ExpressionEntity {
  ...
  constructor(properties, prototypeExpression, immutable = false) {
    ...
    this.allProperties = [];
    this.alwaysIncludedProperties = /* @__PURE__ */ new Set();
    this.deoptimizedPaths = /* @__PURE__ */ new Map();
    this.expressionsToBeDeoptimizedByKey = /* @__PURE__ */ new Map();
    this.gettersByKey = /* @__PURE__ */ new Map();
    this.propertiesAndGettersByKey = /* @__PURE__ */ new Map();
    this.propertiesAndSettersByKey = /* @__PURE__ */ new Map();
    this.settersByKey = /* @__PURE__ */ new Map();
  ...
  deoptimizeArgumentsOnInteractionAtPath(interaction, path, recursionTracker) {
    ...
    (type === INTERACTION_CALLED || path.length > 1) && (this.hasUnknownDeoptimizedProperty || isConcreteKey(key) && this.deoptimizedPaths.get(key))) {
      deoptimizeInteraction(interaction);
      return;
    }
  ...
  deoptimizePath(path) {
    ...
    for (const property2 of isConcreteKey(key) ? [
  ...
  include(context, includeChildrenRecursively) {
    ...
    for (const property2 of this.allProperties) {
      if (includeChildrenRecursively || property2.shouldBeIncluded(context) || this.alwaysIncludedProperties.has(property2)) {
  ...
  includePath(path, context) {
    var _a;
    this.included = true;
    for (const property2 of this.alwaysIncludedProperties) {
      property2.includePath(UNKNOWN_PATH, context);
    }
    if (path.length === 0)
      return;
    const [key, ...subPath] = path;
  ...
  buildPropertyMaps(properties) {
    ...
    for (let index = properties.length - 1; index >= 0; index--) {
      ...
      if (isAnyWellKnown(key) && !TREE_SHAKEABLE_SYMBOLS.has(key)) {
        alwaysIncludedProperties.add(property2);
      ...
      if (isConcreteKey(key)) {
        if (kind === "set") {
          if (!propertiesAndSettersByKey.has(key)) {
            propertiesAndSettersByKey.set(key, [property2, ...unmatchablePropertiesAndSetters]);
            settersByKey.set(key, [property2, ...unmatchableSetters]);
          }
        } else if (kind === "get") {
          if (!propertiesAndGettersByKey.has(key)) {
            propertiesAndGettersByKey.set(key, [property2, ...unmatchablePropertiesAndGetters]);
            gettersByKey.set(key, [property2, ...unmatchableGetters]);
          }
        } else {
          if (!propertiesAndSettersByKey.has(key)) {
            propertiesAndSettersByKey.set(key, [property2, ...unmatchablePropertiesAndSetters]);
  ...
  getMemberExpression(key) {
    if (this.hasLostTrack || this.hasUnknownDeoptimizedProperty || !isConcreteKey(key) || this.hasUnknownDeoptimizedInteger && typeof key === "string" && INTEGER_REG_EXP.test(key) || this.deoptimizedPaths.get(key)) {
    ...
    if (properties || this.unmatchablePropertiesAndGetters.length > 0 || this.unknownIntegerProps.length > 0 && typeof key === "string" && INTEGER_REG_EXP.test(key)) {
      return UNKNOWN_EXPRESSION;
    }
    return null;
  }
...
var OBJECT_PROTOTYPE = new ObjectEntity(/* @__PURE__ */ new Map([
...
var ARRAY_PROTOTYPE = new ObjectEntity(/* @__PURE__ */ new Map([
...
var Identifier2 = class extends IdentifierBase {
  ...
  includeDestructuredIfNecessary(context, destructuredInitPath, init) {
    ...
    if (included || (included = destructuredInitPath.length > 0 && !context.brokenFlow && propertyReadSideEffects && (propertyReadSideEffects === "always" || init.hasEffectsOnInteractionAtPath(destructuredInitPath, NODE_INTERACTION_UNKNOWN_ACCESS, createHasEffectsContext())))) {
...
var MemberExpression2 = class extends NodeBase {
  ...
  applyDeoptimizations() {
    ...
    if (
      ...
      this.bound && propertyReadSideEffects && !(this.variable || this.isUndefined || this.promiseHandler)
    ) {
  ...
  getDynamicPropertyKey() {
    if (this.dynamicPropertyKey === null) {
      ...
      return this.dynamicPropertyKey = typeof value === "symbol" ? WELL_KNOWN_SYMBOLS.has(value) ? value : UnknownKey : String(value);
    }
...
var FILE_OBJ_PREFIX = "ROLLUP_FILE_URL_OBJ_";
var IMPORT = "import";
var MetaProperty = class extends NodeBase {
  ...
  getReferencedFileName(outputPluginDriver) {
    ...
    if (name === IMPORT) {
      if (metaProperty2 == null ? void 0 : metaProperty2.startsWith(FILE_OBJ_PREFIX)) {
        return outputPluginDriver.getFileName(metaProperty2.slice(FILE_OBJ_PREFIX.length));
      } else if (metaProperty2 == null ? void 0 : metaProperty2.startsWith(FILE_PREFIX)) {
  ...
  includeNode() {
    ...
    if (this.meta.name === IMPORT) {
      ...
      if (metaProperty2 == null ? void 0 : metaProperty2.startsWith(FILE_OBJ_PREFIX)) {
        this.referenceId = metaProperty2.slice(FILE_OBJ_PREFIX.length);
      } else if (metaProperty2 == null ? void 0 : metaProperty2.startsWith(FILE_PREFIX)) {
  ...
  render(code, renderOptions) {
    ...
    if (referenceId) {
      ...
      const isUrlObject = !!(metaProperty2 == null ? void 0 : metaProperty2.startsWith(FILE_OBJ_PREFIX));
  ...
  setResolution(format, accessedGlobalsByScope, preliminaryChunkId) {
    ...
    const accessedGlobals = (((_a = this.metaProperty) == null ? void 0 : _a.startsWith(FILE_PREFIX)) || ((_b = this.metaProperty) == null ? void 0 : _b.startsWith(FILE_OBJ_PREFIX)) ? accessedFileUrlGlobals : accessedMetaUrlGlobals)[format];
...
var getResolveUrl = (path, asObject, URL2 = "URL") => `new ${URL2}(${path})${asObject ? "" : ".href"}`;
var getRelativeUrlFromDocument = (relativePath, asObject, umd2 = false) => getResolveUrl(`'${escapeId(relativePath)}', ${umd2 ? `typeof document === 'undefined' ? location.href : ` : ""}document.currentScript && document.currentScript.tagName.toUpperCase() === 'SCRIPT' && document.currentScript.src || document.baseURI`, asObject);
...
var getFileUrlFromFullPath = (path, asObject) => `require('u' + 'rl').pathToFileURL(${path})${asObject ? "" : ".href"}`;
var getFileUrlFromRelativePath = (path, asObject) => getFileUrlFromFullPath(`__dirname + '/${escapeId(path)}'`, asObject);
...
var ExportDefaultVariable = class _ExportDefaultVariable extends LocalVariable {
  constructor(exportDefaultDeclaration2, context) {
    super("default", exportDefaultDeclaration2, exportDefaultDeclaration2.declaration, EMPTY_PATH, context, "other");
    this.hasId = false;
...
var NamespaceVariable = class extends Variable {
  ...
  deoptimizeArgumentsOnInteractionAtPath(interaction, path, recursionTracker) {
    ...
    if (path.length > 1 || path.length === 1 && interaction.type === INTERACTION_CALLED) {
      ...
      if (typeof key === "string") {
        (_a = this.module.getExportedVariablesByName().get(key)) == null ? void 0 : _a.deoptimizeArgumentsOnInteractionAtPath(interaction, path.slice(1), recursionTracker);
      } else {
        deoptimizeInteraction(interaction);
      }
    }
  ...
  deoptimizePath(path) {
    ...
    if (path.length > 1) {
      const key = path[0];
      if (typeof key === "string") {
        (_a = this.module.getExportedVariablesByName().get(key)) == null ? void 0 : _a.deoptimizePath(path.slice(1));
  ...
  renderBlock(options) {
    ...
    const members = [...memberVariables.entries()].filter(([name2, variable]) => !name2.startsWith("*") && variable.included).map(([name2, variable]) => {
...
var getDynamicNamespaceVariable = (namespace) => Object.create(namespace, {
  includePath: {
    value(path, context) {
...
function getExportBlock$1(exports$1, dependencies, namedExportsMode, interop, snippets, t, externalLiveBindings, reexportProtoFromExternal, mechanism = "return ") {
  ...
  if (!namedExportsMode) {
    return `${n2}${n2}${mechanism}${getSingleDefaultExport(exports$1, dependencies, interop, externalLiveBindings, getPropertyAccess)};`;
  }
  let exportBlock = "";
  if (namedExportsMode) {
...
function amd(magicString, { accessedGlobals, dependencies, exports: exports$1, hasDefaultExport, hasExports, id, indent: t, intro, isEntryFacade, isModuleFacade, namedExportsMode, log, outro, snippets }, { amd: amd2, esModule, externalLiveBindings, freeze, generatedCode: { symbols }, interop, reexportProtoFromExternal, strict }) {
  var _a;
  ...
  if (hasExports && (namedExportsMode || ((_a = exports$1[0]) == null ? void 0 : _a.local) === "exports.default")) {
...
function system(magicString, { accessedGlobals, dependencies, exports: exports$1, hasExports, indent: t, intro, snippets, outro, usesTopLevelAwait }, { externalLiveBindings, freeze, name, generatedCode: { symbols }, strict, systemNullSetters }) {
  ...
  magicString.prepend(intro + getHelpersBlock(null, accessedGlobals, t, snippets, externalLiveBindings, freeze, symbols) + getHoistedExportsBlock(exports$1, t, snippets)).append(`${outro}${n2}${n2}` + getSyntheticExportsBlock(exports$1, t, snippets) + getMissingExportsBlock(exports$1, t, snippets)).indent(`${t}${t}${t}`).append(wrapperEnd).prepend(wrapperStart);
}
function analyzeDependencies(dependencies, exports$1, t, { _, cnst, getObject, getPropertyAccess, n: n2 }) {
...
var getStarExcludes = ({ dependencies, exports: exports$1 }) => {
...
var getHoistedExportsBlock = (exports$1, t, snippets) => getExportsBlock(exports$1.filter((expt) => expt.hoisted).map((expt) => ({ name: expt.exported, value: expt.local })), t, snippets);
function getExportsBlock(exports$1, t, { _, n: n2 }) {
  if (exports$1.length === 0) {
    return "";
  }
  if (exports$1.length === 1) {
    return `exports(${JSON.stringify(exports$1[0].name)},${_}${exports$1[0].value});${n2}${n2}`;
  }
  return `exports({${n2}` + exports$1.map(({ name, value }) => `${t}${stringifyObjectKeyIfNeeded(name)}:${_}${value}`).join(`,${n2}`) + `${n2}});${n2}${n2}`;
}
var getSyntheticExportsBlock = (exports$1, t, snippets) => getExportsBlock(exports$1.filter((expt) => expt.expression).map((expt) => ({ name: expt.exported, value: expt.local })), t, snippets);
var getMissingExportsBlock = (exports$1, t, snippets) => getExportsBlock(exports$1.filter((expt) => expt.local === MISSING_EXPORT_SHIM_VARIABLE).map((expt) => ({ name: expt.exported, value: MISSING_EXPORT_SHIM_VARIABLE })), t, snippets);
...
function umd(magicString, { accessedGlobals, dependencies, exports: exports$1, hasDefaultExport, hasExports, id, indent: t, intro, namedExportsMode, log, outro, snippets }, { amd: amd2, compact, esModule, extend, externalLiveBindings, freeze, interop, name, generatedCode: { symbols }, globals, noConflict, reexportProtoFromExternal, strict }) {
  var _a;
  ...
  if ((hasExports || noConflict) && (namedExportsMode || hasExports && ((_a = exports$1[0]) == null ? void 0 : _a.local) === "exports.default")) {
...
var ObjectPattern = class extends NodeBase {
  ...
  includeDestructuredIfNecessary(context, destructuredInitPath, init) {
    ...
    for (const property2 of this.properties.slice(0, -1)) {
      if (lastPropertyIsRestElement && included && !property2.included) {
...
var AssignmentExpression = class extends NodeBase {
  ...
  hasEffectsOnInteractionAtPath(path, interaction, context) {
    return interaction.type === INTERACTION_ASSIGNED && this.left.included || this.right.hasEffectsOnInteractionAtPath(path, interaction, context);
  }
  include(context, includeChildrenRecursively) {
    var _a;
...
var ObjectPromiseHandler = class {
  ...
  deoptimizeArgumentsOnInteractionAtPath(interaction, path, recursionTracker) {
    deoptimizeInteraction(interaction);
    if (interaction.type === INTERACTION_CALLED && path.length === 0 && (isFunctionExpressionNode(interaction.args[1]) || isArrowFunctionExpressionNode(interaction.args[1]))) {
      interaction.args[1].deoptimizeArgumentsOnInteractionAtPath(this.interaction, [], recursionTracker);
    }
  }
  includeCallArguments(interaction, context) {
    includeInteractionWithoutThis(interaction, context);
    if (interaction.type === INTERACTION_CALLED && (isFunctionExpressionNode(interaction.args[1]) || isArrowFunctionExpressionNode(interaction.args[1]))) {
      interaction.args[1].includeCallArguments(this.interaction, context);
    }
  }
};
var EmptyPromiseHandler = class {
...
var ImportExpression = class extends NodeBase {
  ...
  bind() {
    ...
    let callExpression2 = this;
    while (true) {
      if (currentParent.computed || currentParent.object !== callExpression2 || !isIdentifierNode(currentParent.property) || !isCallExpressionNode(currentParent.parent)) {
        break;
      }
      const propertyName = currentParent.property.name;
      callExpression2 = currentParent.parent;
      if (propertyName === "then") {
        const firstArgument = callExpression2.arguments[0];
        if (firstArgument === void 0 || isFunctionExpressionNode(firstArgument) || isArrowFunctionExpressionNode(firstArgument)) {
        ...
        if (isMemberExpressionNode(callExpression2.parent)) {
          currentParent.promiseHandler = new EmptyPromiseHandler();
          currentParent = callExpression2.parent;
          continue;
        }
        if (isExpressionStatementNode(callExpression2.parent)) {
          currentParent.promiseHandler = new EmptyPromiseHandler();
          this.localResolution = { resolution, tracked: true };
          return;
        }
  ...
  deoptimizePath(path) {
    var _a, _b;
    (_b = (_a = this.localResolution) == null ? void 0 : _a.resolution) == null ? void 0 : _b.namespace.deoptimizePath(path);
  }
  hasEffects() {
    return true;
  }
  include(context, includeChildrenRecursively) {
    ...
    if (this.shouldIncludeDynamicAttributes) {
      (_a = this.options) == null ? void 0 : _a.include(context, includeChildrenRecursively);
    }
    if (includeChildrenRecursively) {
      (_b = this.localResolution) == null ? void 0 : _b.resolution.includeAllExports();
    }
  }
  includeNode(context) {
    var _a;
    ...
    if (shouldIncludeDynamicAttributes) {
      (_a = this.options) == null ? void 0 : _a.includePath(UNKNOWN_PATH, context);
    }
  ...
  includePath(path, context) {
    ...
    if (!this.included)
      this.includeNode(context);
    (_b = (_a = this.localResolution) == null ? void 0 : _a.resolution) == null ? void 0 : _b.namespace.includeMemberPath(path, context);
  }
  initialise() {
    super.initialise();
    ...
    if (["classic", "automatic"].includes(jsxMode)) {
      ...
      if (!(jsxMode === "automatic" && key === "key")) {
        ...
        if (value) {
          ...
          if (value instanceof Literal2 && typeof value.value === "string" && value.value.includes("\n")) {
...
var RE_WHITESPACE_TRIM = /^[ \t]*\r?\n[ \t\r\n]*|[ \t]*\r?\n[ \t\r\n]*$/g;
var RE_WHITESPACE_MERGE = /[ \t]*\r?\n[ \t\r\n]*/g;
var JSXText = class extends NodeBase {
  ...
  getRenderedText() {
    if (this.renderedText === void 0)
      this.renderedText = this.value.replace(RE_WHITESPACE_TRIM, "").replace(RE_WHITESPACE_MERGE, " ");
...
function getRenderedJsxChildren(children) {
  ...
  for (const child of children) {
    if (!(child instanceof JSXExpressionContainer && child.expression instanceof JSXEmptyExpression) && (!(child instanceof JSXText) || child.shouldRender())) {
      renderedChildren++;
    }
  }
...
var LogicalExpression = class extends NodeBase {
  ...
  getUsedBranch() {
    if (!this.isBranchResolutionAnalysed) {
      ...
      if (typeof booleanOrUnknown === "symbol" || this.operator === "??" && typeof leftValue === "symbol") {
        return null;
      } else {
...
var ModuleScope = class extends ChildScope {
  ...
  addDeclaration(identifier2, context, init, destructuredInitPath, kind) {
    if (this.importDescriptions.has(identifier2.name)) {
      context.error(logRedeclarationError(identifier2.name), identifier2.start);
    }
    return super.addDeclaration(identifier2, context, init, destructuredInitPath, kind);
  }
  addExportDefaultDeclaration(exportDefaultDeclaration2, context) {
    const variable = new ExportDefaultVariable(exportDefaultDeclaration2, context);
...
var VariableDeclarator2 = class extends NodeBase {
  ...
  hasEffects(context) {
    ...
    this.id.markDeclarationReached();
    return initEffect || this.isUsingDeclaration || this.isAsyncUsingDeclaration || this.id.hasEffects(context) || this.scope.context.options.treeshake.propertyReadSideEffects && this.id.hasEffectsWhenDestructuring(context, EMPTY_PATH, this.init || UNDEFINED_EXPRESSION);
  }
  include(context, includeChildrenRecursively) {
    const { id, init } = this;
    if (!this.included)
...
var SYMBOL_DISPOSE_PATH = [SymbolDispose];
var SYMBOL_ASYNC_DISPOSE_PATH = [SymbolAsyncDispose];
...
function getVariableForExportNameRecursive(target, name, importerForSideEffects, isExportAllSearch, searchedNamesAndModules = /* @__PURE__ */ new Map(), importChain) {
...
var Module = class _Module {
  constructor(graph, id, options, isEntry, moduleSideEffects, syntheticNamedExports, meta, attributes) {
    ...
    this.includedTopLevelAwaitingDynamicImporters = /* @__PURE__ */ new Set();
    ...
    this.exportDescriptions = /* @__PURE__ */ new Map();
  ...
  getExportedVariablesByName() {
    ...
    const exportedVariablesByName = this.exportedVariablesByName = /* @__PURE__ */ new Map();
  ...
  getRenderedExports() {
    ...
    for (const exportName of this.exportDescriptions.keys()) {
      (((_a = this.getExportedVariablesByName().get(exportName)) == null ? void 0 : _a.included) ? renderedExports : removedExports).push(exportName);
    }
  ...
  getVariableForExportName(name, { importerForSideEffects, importChain = [], isExportAllSearch, onlyExplicit, searchedNamesAndModules } = EMPTY_OBJECT) {
    ...
    if (name !== "default") {
      const foundNamespaceReexport = this.namespaceReexportsByName.get(name) ?? this.getVariableFromNamespaceReexports(name, importerForSideEffects, searchedNamesAndModules, [...importChain, this.id]);
...
var sortExportedVariables = ([a], [b]) => a < b ? -1 : a > b ? 1 : 0;
...
function deconflictTopLevelVariables(usedNames, modules, includedNamespaces) {
  var _a, _b;
  for (const module of modules) {
    (_a = module.info).safeVariableNames || (_a.safeVariableNames = {});
    for (const variable of module.scope.variables.values()) {
      ...
      !(variable.renderBaseName || variable instanceof ExportDefaultVariable && variable.getOriginalVariable() !== variable)) {
        const cachedSafeVariableName = (_b = Object.getOwnPropertyDescriptor(module.info.safeVariableNames, variable.name)) == null ? void 0 : _b.value;
...
`);
      ...
      if (addNonNamespacesAndInteropHelpers || variable.isNamespace) {
        ...
        if (module instanceof ExternalModule) {
          ...
          if (addNonNamespacesAndInteropHelpers) {
            ...
            } else if (variable.isNamespace && namespaceInteropHelpersByInteropType[interop(module.id)] && (this.imports.has(variable) || !((_a = this.exportNamesByVariable.get(variable)) == null ? void 0 : _a.every((name) => name[0] === "*")))) {
    ...
    for (const resolvedDynamicImport of this.getIncludedDynamicImports()) {
      if (resolvedDynamicImport.chunk) {
        ...
        if (chunk === this) {
          ...
          node.setExternalResolution((facadeChunk || chunk).exportMode, outputOptions, snippets, pluginDriver, accessedGlobalsByScope, `'${(facadeChunk || chunk).getImportPath(fileName)}'`, !(facadeChunk == null ? void 0 : facadeChunk.strictFacade) && chunk.exportNamesByVariable.get(resolution.namespace)[0], null, this, facadeChunk || chunk);
        }
      } else {
  ...
  setUpChunkImportsAndExportsForModule(module) {
    ...
    for (const { node: { included, resolution } } of module.dynamicImports) {
      if (included && resolution instanceof Module && this.chunkByModule.get(resolution) === this && !this.includedNamespaces.has(resolution)) {
...
function getChunkDefinitionsFromManualChunks(manualChunkAliasByEntry, isManualChunksFunctionForm, onlyExplicitManualChunks) {
  ...
  for (const [entry, alias] of sortedEntriesWithAlias) {
    if (isManualChunksFunctionForm && onlyExplicitManualChunks) {
      (manualChunkModulesByAlias[alias] || (manualChunkModulesByAlias[alias] = [])).push(entry);
    } else {
  ...
  checkCircularChunks(chunks) {
      ...
      if (parents.has(chunk)) {
        if (!visited.has(chunk)) {
          ...
          while (nextChunk !== chunk && nextChunk) {
            ...
            isManualChunkConflict && (isManualChunkConflict = nextChunk.isManualChunk);
    ...
    const analyseChunk = (chunk) => {
      for (const dependency of chunk.dependencies) {
        if (dependency instanceof Chunk2) {
...
function isFileNameOutsideOutputDirectory(fileName) {
  const normalized = (0, import_node_path.join)(fileName).replaceAll("\\", "/");
  return normalized === ".." || normalized.startsWith("../") || normalized === "." || isAbsolute(normalized);
}
...
function resolveIdViaPlugins(source, importer, pluginDriver, moduleLoaderResolveId, skip, customOptions, isEntry, attributes, importerAttributes) {
  ...
  if (skip) {
    ...
    replaceContext = (pluginContext, plugin) => ({
      ...pluginContext,
      resolve: (source2, importer2, { attributes: attributes2, custom, isEntry: isEntry2, skipSelf, importerAttributes: importerAttributes2 } = BLANK) => {
        ...
        return moduleLoaderResolveId(source2, importer2, custom, isEntry2, attributes2 || EMPTY_OBJECT, importerAttributes2, skipSelf ? [...skip, { importer: importer2, plugin, source: source2 }] : skip);
      }
    });
  }
...
async function resolveId(source, importer, preserveSymlinks, pluginDriver, moduleLoaderResolveId, skip, customOptions, isEntry, attributes, importerAttributes, fs2) {
...
var ModuleLoader = class {
  constructor(graph, modulesById, options, pluginDriver) {
    ...
    this.resolveId = async (source, importer, customOptions, isEntry, attributes, importerAttributes, skip = null) => this.getResolvedIdWithDefaults(this.getNormalizedResolvedIdWithoutDefaults(this.options.external(source, importer, false) ? false : await resolveId(source, importer, this.options.preserveSymlinks, this.pluginDriver, this.resolveId, skip, customOptions, typeof isEntry === "boolean" ? isEntry : !importer, attributes, importerAttributes, this.options.fs), importer, source), attributes);
  ...
  async addEntryModules(unresolvedEntryModules, isUserDefined) {
    ...
    const newEntryModules = await this.extendLoadModulesPromise(Promise.all(unresolvedEntryModules.map(({ id, importer }) => this.loadEntryModule(id, true, importer, null, void 0, void 0))).then((entryModules) => {
  ...
  addEntryWithImplicitDependants(unresolvedModule, implicitlyLoadedAfter) {
    ...
    return this.extendLoadModulesPromise(this.loadEntryModule(unresolvedModule.id, false, unresolvedModule.importer, null, void 0, void 0).then(async (entryModule) => {
      addChunkNamesToModule(entryModule, unresolvedModule, false, chunkNamePriority);
      if (!entryModule.info.isEntry) {
        const implicitlyLoadedAfterModules = await Promise.all(implicitlyLoadedAfter.map((id) => this.loadEntryModule(id, false, unresolvedModule.importer, entryModule.id, void 0, void 0)));
        if (!entryModule.info.isEntry) {
  ...
  getResolveStaticDependencyPromises(module) {
    return Array.from(module.sourcesWithAttributes, async ([source, attributes]) => [
      source,
      module.resolvedIds[source] = module.resolvedIds[source] || this.handleInvalidResolvedId(await this.resolveId(source, module.id, EMPTY_OBJECT, false, attributes, module.info.attributes), source, module.id, attributes)
    ]);
  }
...
async function watchInternal(configs, emitter) {
  ...
  await loadFsEvents();
  const { Watcher } = await import("./watch-UDQLWSVU.js");
...
rollup/dist/es/shared/node-entry.js:
  ...
  *)
*/
//# sourceMappingURL=chunk-7W567IA6.js.map
